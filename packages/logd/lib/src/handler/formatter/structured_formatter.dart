  ...
  LogDocument format(
    ...
    if (metadata.contains(LogMetadata.timestamp)) {
      nodes.add(
        _buildHeaderNode([
          StyledText(
            entry.timestamp,
            tags: const {LogTag.timestamp, LogTag.header},
          ),
        ]),
      );
    }
    ...
    final levelLoggerSegments = [
      StyledText(
        ...
        tags: const {LogTag.level, LogTag.header},
      ),
    ];
    if (metadata.contains(LogMetadata.logger)) {
      levelLoggerSegments
        ...
        ..add(
          StyledText(
            '[${entry.loggerName}]',
            tags: const {LogTag.loggerName, LogTag.header},
          ),
        );
    }
    ...
    if (metadata.contains(LogMetadata.origin)) {
      nodes.add(
        _buildHeaderNode([
          StyledText(
            '[${entry.origin}]',
            tags: const {LogTag.origin, LogTag.header},
          ),
        ]),
      );
    }
            ...
            children: [
              ParagraphNode(
                children: [
                  FooterNode(
                    segments: [
                      StyledText(text, tags: const {LogTag.stackFrame}),
                    ],
                  ),
                ],
              ),
            ...
            children: [
              ParagraphNode(
                children: [
                  FooterNode(
                    segments: [
                      StyledText(raw, tags: const {LogTag.stackFrame}),
                    ],
                  ),
                ],
              ),
