...
final entry = LogEntry(
  ...
  origin: 'three_engines_comparison.dart:10',
);
...
Future<void> main() async {
  print('# Logd Three-Engine Performance Comparison');

  final info = await Service.getInfo();
  if (info.serverUri == null) {
    print('Error: VM Service not enabled. Run with:');
    print('  dart run --enable-vm-service lib/three_engines_comparison.dart');
    return;
  }
  ...
  final scenarios = [
    Scenario('1. Plain Text (Compact)', const PlainFormatter(), []),
  ...
  print('\n\n${'=' * 60}');
  print('FINAL SYSTEM COMPARISON SUMMARY');
  print('=' * 60);
  ...
  for (final entry in report.entries) {
    ...
    for (final res in entry.value) {
      print(
          '| **${res.engine}** | ${res.opsPerSec.toStringAsFixed(0)} | ${res.p90.toStringAsFixed(1)} | ${res.gcPressureKb} |');
    }
  }
...
Future<Result> profile(
  ...
  // Use a standard EncodingSink without custom output overrides
  // to run the actual production serialization pipeline on both paths.
  final sink = BenchmarkEncodingSink(width: scenario.width);
...
base class BenchmarkEncodingSink extends EncodingSink {
  BenchmarkEncodingSink({required final int width})
      : super(
  ...
  static void _blackhole(Uint8List data) {}
}
